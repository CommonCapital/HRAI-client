...
const pricingCardVariants = cva("rounded-lg p-4 py-6 w-full", {
    variants: {
        variant: {
            ...
            highlighted: "bg-radial from-blue-800 to blue-950 text-white",
            enterprise: "bg-radial from-gray-500 to gray-700 text-black border border-gray-500 hover:bg-gray-600",
        }
    },
     defaultVariants: {
            variant: "default",
...
const pricingIconCardVariants = cva("size-5", {
    variants: {
        variant: {
            ...
            enterprise: "fill-white text-white",
        }
    },
     defaultVariants: {
            variant: "default",
...
const pricingCardBadgeVariants = cva("text-black text-xs font-normal p-1", {
    variants: {
        variant: {
            default: "bg-primary",
            highlighted: "bg-[#5D6B68]",
            enterprise: "bg-[#5D6B68]",
        }
    },
     defaultVariants: {
            variant: "default",
...
export const PricingCard = ({
    badge, price, variant, features, title, description, priceSuffix, className, buttonText, onClick
}: Props) => {
  return (
    <div className={cn(pricingCardVariants({variant}), className, "border")}>
      <div className="flex items-end gap-x-4 justify-between">
        <div className="flex flex-col gap-y-2">
            ...
            <p 
             className={cn(pricingCardSecondaryTextVariants({variant}))}
            >
                {description}
            </p>
        ...
        <div className="flex items-end shrink-0 gap-x-0.5">
           ...
           {typeof price === 'number' ? Intl.NumberFormat("en-US", {
  style: "currency",
  currency: "USD",
  minimumFractionDigits: 0,
}).format(price) : 'Contract'}
           </h4>
  ...
  {( variant === "default") ? (
    ...
    <Separator className="text-yellow-400" />
  )}
</div>

<Button className="w-full" size="lg" variant={variant === "enterprise" || variant === "highlighted" ? "default" : "outline"} onClick={onClick}>
    {buttonText}
</Button>
